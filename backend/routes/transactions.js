...
// Import SplitService for split-related operations
 ...
 * @access  Private
 */
router.put('/:id/split', async (req, res, next) => {
  try {
    const transactionId = req.params.id;
 ...
 * @access  Private
 */
router.delete('/:id/split', async (req, res, next) => {
  try {
    const transactionId = req.params.id;
 ...
 * @access  Private
 */
router.post('/:id/split/settle/:userId', async (req, res, next) => {
  try {
    const transactionId = req.params.id;
...
/**
 * @route   GET /api/transactions/balance/:friendId
 * @desc    Get detailed balance between current user and a friend
 * @access  Private
 */
router.get('/balance/:friendId', async (req, res, next) => {
  try {
    const userId = req.user.id;
    const friendId = req.params.friendId;

    const balanceDetails = await SplitService.getDetailedBalance(userId, friendId);
    ...
    console.error('Get detailed balance error:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to get balance details'
    });
  }
});

/**
 * @route   GET /api/transactions/split-summary
 * @desc    Get split transaction summary for current user
 * @access  Private
 */
router.get('/split-summary', async (req, res, next) => {
  try {
    const userId = req.user.id;
    ...
    const summary = await SplitService.getUserSplitSummary(userId, filters);
    ...
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to get split summary'
    });
  }
});
